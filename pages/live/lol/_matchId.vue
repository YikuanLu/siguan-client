<template>
  <div>
    <div class="wrap-box">
      ...
      <LiveHeader :type="'lol'" :data="baseInfo"></LiveHeader>
      <LivePlatformTabs :type="'lol'"></LivePlatformTabs>
      <LiveGoing :type="'lol'"></LiveGoing>
      <div class="content clearfix">
        <div class="left">
          ...
          <div v-if="cur === 1 || cur === 4">
            ...
            <div v-if="showLive && Object.keys(LiveData).length" class="round-box">
              <GameStatus :battleData="LiveData"></GameStatus>
              <div class="chart-box">
                <EconomicGap :battleData="LiveData" :data="baseInfo" class="economic-gap"></EconomicGap>
                <GameDataRatio :battleData="LiveData" class="gamedata-ratio"></GameDataRatio>
              </div>
              <PlayerData :battleData="LiveData"></PlayerData>
            </div>
          ...
          <div v-if="cur === 2">
            <div v-if="showBet && Object.keys(betData).length" class="round-box">
              ...
              <StaticTable :data="betData" :type="'lol'" :baseInfo="baseInfo"></StaticTable>
            </div>
        ...
        <div class="right">
          ...
          <sider-box class="sider" :title="'LOL战队KDA排行'">
            <SiderScoreRank gameType="lol" ></SiderScoreRank>
          </sider-box>
          <sider-box class="sider" :title="'英雄联盟视频'" :moreLink="'/'">
            <SiderVideos></SiderVideos>
          </sider-box>
          <sider-box class="sider" :title="'英雄联盟图集'" :moreLink="'/'">
            <SiderPictures></SiderPictures>
          </sider-box>
          <sider-box class="sider" :title="'LOL最新资讯'" :moreLink="'/news'">
...
<script>
import { importComponents } from '~/components/live/lol/components'
...
export default {
  ...
  async asyncData ({ $axios, params }) {
    ...
    const [
      ...
      $axios.get(matchApi.lol.match.basicInfo, { params: baseInfoParams })
    ])
  ...
  methods: {
    ...
    getBasicInfo () {
      ...
      this.$axios.get(matchApi.lol.match.basicInfo, { params }).then((res) => {
        this.baseInfo = res
      })
    },
    initTabOrder () {
    ...
    getCurBattle () {
      ...
      const curRound = this.baseInfo.battle_list.filter(_ => _.status === 0)
      if (curRound[0]) {
        this.curBattleId = curRound[0].battle_id
      } else {
        const len = this.baseInfo.battle_list.length
        try {
          this.curBattleId = this.baseInfo.battle_list[ len - 1 ].battle_id
        } catch (error) {
    ...
    getLiveData () {
      ...
      this.$axios.get(matchApi.lol.match.liveBattle, { params }).then((res) => {
        this.LiveData = res
        // 获取完数据才能实例化组件，否则报错
        this.showLive = true
      })
    ...
    getStaticData () {
      ...
      this.$axios.get(matchApi.lol.match.betInfoList, { params }).then((res) => {
        this.betData = res
        // 获取完数据才能实例化组件，否则报错
        this.showBet = true
      })
    ...
    getProSpectData () {
      ...
      this.$axios.get(matchApi.lol.match.proSpect, { params }).then((res) => {
    ...
    initBreadCrumb () {
      try {
        ...
        const league = {
          ...
          path: `/match/lol/league/${this.baseInfo.leagueId}`
        }
        const vsTeam = {
