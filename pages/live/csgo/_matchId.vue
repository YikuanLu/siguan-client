<template>
  <div>
    <div class="wrap-box">
      ...
      <LiveHeader :type="'csgo'" :data="baseInfo"></LiveHeader>
      <div class="content clearfix">
        <div class="left">
          ...
          <div v-if="cur === 1 || cur === 4">
            <LiveRoundTabs :data="baseInfo.battle_list" :index="curBattleId" @switchTab="switchRound"></LiveRoundTabs>
            <div v-if="showLive && Object.keys(LiveData).length" class="round-box">
              <LivesScore :data="LiveData"></LivesScore>
              <GameStatus v-if="cur === 1" :data="LiveData"></GameStatus>
              <AFGameStatus v-if="cur === 4" :data="LiveData"></AFGameStatus>
          ...
          <div v-if="cur === 2">
            <div v-if="showBet && Object.keys(betData).length" class="round-box">
              ...
              <StaticTable :data="betData" :type="'csgo'" :baseInfo="baseInfo"></StaticTable>
            </div>
          ...
          <div v-if="cur === 3">
            <div v-if="showProSpect && Object.keys(proSpectData).length" class="round-box">
              <div class="item-box">
                <p class="common-tit">地图胜率(胜率/局数)</p>
                <MapWinRate :data="proSpectData"></MapWinRate>
              </div>
              <div class="item-box">
        ...
        <div class="right">
          ...
          <sider-box class="sider" :title="'CSGO职业战队实时排行'">
            <SiderScoreRank gameType="csgo"></SiderScoreRank>
          </sider-box>
          <sider-box class="sider" :title="'反恐精英视频'" :moreLink="'/'">
            <SiderVideos></SiderVideos>
          </sider-box>
          <sider-box class="sider" :title="'反恐精英图集'" :moreLink="'/'">
            <SiderPictures></SiderPictures>
          </sider-box>
          <sider-box class="sider" :title="'CSGO最新资讯'" :moreLink="'/news'">
...
<script>
import { importComponents } from '~/components/live/csgo/components'
...
export default {
  ...
  head () {
    const { baseInfo } = this
    const { leagueName, teamAName, teamBName } = baseInfo
    return {
      title: `${leagueName} ${teamAName}:${teamBName}|实时比分直播|比赛结果|历史比分数据分析 - 尚牛电竞`,
      meta: [
  ...
  async asyncData ({ $axios, params }) {
    ...
    const [
      ...
      $axios.get(matchApi.csgo.match.basicInfo, { params: matchIdParams })
    ])
  ...
  methods: {
    ...
    getBasicInfo () {
      ...
      this.$axios.get(matchApi.csgo.match.basicInfo, { params }).then((res) => {
        this.baseInfo = res
      })
    },
    initTabOrder () {
    ...
    getCurBattle () {
      ...
      if (len > 0) {
        this.curBattleId = this.baseInfo.battle_list[ len - 1 ].sortNo
      } else {
    ...
    async getLiveData () {
      ...
      const params = Object.assign(this.$route.params, {
        index: this.curBattleId
      })
      await this.$axios.get(matchApi.csgo.match.liveBattle, { params }).then((res) => {
        this.LiveData = res
      })

      const logParams = this.$route.params
      await this.$axios.get(matchApi.csgo.match.battleLog, { params: logParams }).then((res) => {
        this.liveText = res
      })
    ...
    getStaticData () {
      ...
      this.$axios.get(matchApi.csgo.match.betInfo, { params }).then((res) => {
        this.betData = res
        // 获取完数据才能实例化组件，否则报错
        this.showBet = true
      })
    ...
    getProSpectData () {
      ...
      this.$axios.get(matchApi.csgo.match.prospect, { params }).then((res) => {
    ...
    initBreadCrumb () {
      try {
        ...
        const league = {
          ...
          path: `/match/csgo/league/${this.baseInfo.leagueId}`
        }
        const vsTeam = {
