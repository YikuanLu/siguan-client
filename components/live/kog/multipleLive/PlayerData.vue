...
export default {
  ...
  render () {
    ...
    function centerLineData (title, name, teamA, teamB) {
      if (name === 'no-data' || (!teamA[name] && !teamB[name])) {
        return (
          <div>
      ...
      return (
        <div>
          <p class="title">{title}</p>
          <div class="ratio">
            <span class="num">{teamA[name].toFixed(0)}{name === 'offered_rate' && '%'}</span>
            <div class="percent-box">
            ...
            <span class="num">{teamB[name].toFixed(0)}{name === 'offered_rate' && '%'}</span>
          </div>
        </div>
      )
    }
    ...
    const centerInfo = (item) => {
      ...
      return (
        <div class="center-info">
          ...
          {centerLineData('参团率', 'offered_rate', item.blue, item.red)}
        </div>
      )
    }
