  ...
  render () {
    ...
    const title = (
      <div class="history-title">
        <div class="team"
          onClick={ () => {
            openMatchTeamUrl(this, { gameType: 'lol', id: teams.teamAId })
          }}>
          <span class="team-name">{teams.teamAShortName}</span>
        ...
        <div class="team"
          onClick={ () => {
            openMatchTeamUrl(this, { gameType: 'lol', id: teams.teamBId })
          }}>
          <img class="team-logo" src={teams.teamBLogo} alt=""/>
          <span class="team-name">{teams.teamBShortName}</span>
        </div>
      </div>
    )
        ...
        <tbody>
          {allData.map((item) => {
            return (
              <tr>
                <td width="8%">
                  <GameTypeIcon class="game-icon" type="lol"/>
                </td>
                <td width="20%" align="left" title={item.leagueName}>
                  <span class="game-name"
                    onClick={ () => {
                      openMatchLeagueUrl(this, { gameType: 'lol', id: item.leagueId })
                    }}
                ...
                <td>
                  <a-button class="game-link">
                    <nuxt-link to={`/live/lol/${item.matchId}?tab=4`} target="_blank">数据</nuxt-link>
                  </a-button>
                  <a-button class="game-link">
                    <nuxt-link to={`/live/lol/${item.matchId}?tab=2`} target="_blank">指数</nuxt-link>
                  </a-button>
                </td>
              </tr>
            )
