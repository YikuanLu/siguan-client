...
import StageGroupTable from '~/components/match/league/stageGroupTable'
    ...
    getStageGroup () {
      ...
      this.apiByGameType.league.stageGroup(params).then((res) => {
        for (const item of res) {
          item.name = item.stage || '联赛'
        }
        this.stageGroupList = res
        if (this.stageGroupList.length > 0) {
          this.currentStageGroup = this.stageGroupList[0]
        }
      })
    },
    // 获取本周赛程
  ...
  render () {
    ...
    const stageGroupVm = (
      <TabList
        ...
        on-change={(val) => {
          this.currentStageGroup = stageGroupList[val]
          this.stageGroupIndex = val
        }}
      >
        <StageGroupTable
          apiGroup={apiByGameType.league}
        ...
        <div class="dataListBox">
          ...
          >
            <MatchTableList
              ...
              scroll={analysisCurIndex === 2 ? { x: 1200 } : { x: 2500 }}
            />
          </TabList>
        </div>
      </div>
